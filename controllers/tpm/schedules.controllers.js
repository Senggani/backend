...
module.exports = {
  ...
  getMachineList: async (req, res) => {
    try {
      ...
      response.success(res, "success to get visualization of item check", cons);
    } catch (error) {
      // console.log(error);
      response.failed(res, 'Error to get data testing machine list')
    }
  },
  ...
  getItemList: async (req, res) => {
    try {
      ...
      let itemm = req.query.machine

      let machine_q = `
      SELECT DISTINCT
        item_check as item_nm,
        item_check as item_id
      FROM 
        z_history_ireporter 
      WHERE
        equipment_name = '${itemm}'
      ORDER BY 
        item_check ASC
      `
      ...
      // console.log(error);
      response.failed(res, 'Error to get data testing item list')
    }
  },
  ...
  getTestData: async (req, res) => {
    try {
      ...
      let machine_id = req.query.machine
      ...
      let q = `
        SELECT 
            ...
            evaluation as standard_value,
            evaluation as min_std,
            evaluation as max_std,
            checked_time as months
        FROM 
            z_history_ireporter
        WHERE
          equipment_name = '${machine_id}' AND
          item_check = '${item_id}'
        `
      ...
      cons.forEach(cons => {
        series[0].data.push(cons.standard_value ?? 0);
        series[1].data.push((cons.min_std) ?? 0);
        series[2].data.push((cons.max_std) ?? 0);
        labels.push(cons.months);
        // // console.log(schedule);
      });

      // console.log(cons);

      const visualizeHistoryData = {
        series,
        labels
      };

      response.success(res, "success to get visualization of item check", visualizeHistoryData);

    } catch (error) {
      // console.log(error);
      response.failed(res, 'Error to get data testing')
    }
  },
  ...
  updateTestData: async (req, res) => {
    try {
      ...
      let q = `
        INSERT INTO z_history_ireporter (
          equipment_id,
          equipment_name,
          core_equipment,
          line,
        ...
        VALUES (
          ...
          '${req.query.equipment_name}',
          '${req.query.core_equipment}',
          '${req.query.line}',
          '${req.query.station}',
          '${req.query.no_kanban}',
          '${req.query.working_shift}',
          '${req.query.periodical_check}',
          '${req.query.technician_name}',
          '${req.query.checked_name}',
          CURRENT_DATE,
          '${req.query.item_check}',
          '${req.query.evaluation}',
          20200709131021
        )
        `
      queryCustom(q)
      response.success(res, `success to insert check data ${req.query.technician_name}`);

    } catch (error) {
      // console.log(error);
      response.failed(res, 'Error to insert check data')
    }
  },
